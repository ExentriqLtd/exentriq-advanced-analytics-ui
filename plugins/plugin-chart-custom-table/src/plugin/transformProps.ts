...
const isEqualColumns = <T extends TableChartProps[]>(propsA: T, propsB: T) => {
  ...
  return (
    ...
    a.formData.numberFormat === b.formData.numberFormat &&
...
const processColumns = memoizeOne(function processColumns(props: TableChartProps) {
  ...
  const columns: DataColumnMeta[] = (columns_ || []).map((key: string) => {
    ...
    const isNumericCol = isNumeric(key, records);
    ...
    } else if (isMetric || isNumericCol) {
      console.log(key);
      // console.log(format);
      // console.log(NumberFormats.FLOAT_1_POINT);
      // console.log(NumberFormats.FLOAT_2_POINT);
      // console.log(NumberFormats.FLOAT_3_POINT);
      // console.log(NumberFormats.FLOAT);
      console.log(numberFormat);
      formatter = getNumberFormatter(format || numberFormat || NumberFormats.FLOAT);
